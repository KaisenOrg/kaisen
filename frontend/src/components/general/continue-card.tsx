...
const COURSE_DESCRIPTION =
  "Master Python step by step with personalized, interactive content";
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col text-left text-white px-6">
          <div className="flex">
            <BookmarkIcon
              ...
              strokeWidth={2}
            />
            <h2 className="text-base font-medium text-white pb-2">
              {COURSE_TITLE}
            </h2>
          </div>
          <p className={cn("text-sm text-zinc-400 pb-6", descriptionClassName)}>
            {COURSE_DESCRIPTION}
          </p>
        </div>
        {showView && (
          <Button
            variant="outline"
            className="mx-auto max-w-[120px] w-full border border-zinc-800"
          >
            View
          </Button>
        )}
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-zinc-800 rounded-b-lg">
        <Progress
          value={progress ? progress : PROGRESS_VALUE}
          indicatorClassName={`bg-green-400 ${isCompleted && "bg-orange-500"}`}
