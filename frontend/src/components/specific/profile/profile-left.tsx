...
const ProfileLeft = () => {
	...
	const avatarUrl = user?.picture || "/assets/profile-placeholder.png";
	const initials = user?.nickname?.slice(0, 2).toUpperCase() || "U";
	const displayName = user?.nickname || "Usuário";
	const username = user?.username ? `@${user.username}` : "@username";
	const bio = user?.about || "No bio provided.";
	const followers = user?.followers?.length ?? 0;
	const following = user?.following?.length ?? 0;
	// Socials (optional, fallback to #)
	const github = "#";
	const linkedin = "#";
		...
		<aside className="w-full max-w-sm text-white rounded-xl shadow-lg p-6 flex flex-col items-start">
			...
			<div className="flex items-center w-full mb-4">
				...
				<div className="flex flex-col justify-center">
					<h2 className="text-2xl font-semibold text-left">{displayName}</h2>
					<span className="text-base text-zinc-400">{username}</span>
				</div>
			</div>
			...
			<div className="flex gap-4 mb-4 w-full">
				<span className="font-semibold font-base text-left">{followers} <span className="text-zinc-400 font-normal">Followers</span></span>
				<span className="font-semibold font-base text-left">{following} <span className="text-zinc-400 font-normal">Following</span></span>
			</div>
			{/* Buttons */}
